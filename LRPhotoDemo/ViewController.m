...
@property (weak, nonatomic) IBOutlet UITextView *LRTextView;
...
- (void)viewDidLoad {
    self.LRTextView.editable = YES;
    //self.LRTextView.text = @"开始监听";
    //self.globalString =@"开始监听";
    //NSString *str = self.LRTextView.text;
//    [NSThread sleepForTimeInterval:2];
//    self.LRTextView.text = [str stringByAppendingString:@"\n123123\n"];
//    [NSThread sleepForTimeInterval:2];
//    self.LRTextView.text = [str stringByAppendingString:@"test\n"];
...
- (IBAction)openAlbum:(UIButton *)sender {
    ...
    //[self showPhotosManager];
    NSInteger changeCount = [UIPasteboard generalPasteboard].changeCount;
    self.globalString = [NSString stringWithFormat:@"%d",changeCount];
    self.LRTextView.text = self.globalString;
...
//        NSString *temptext = @"监听到相册变化\n\n\n";
//        self.globalString = [temptext stringByAppendingString:self.globalString];
//        self.LRTextView.text = [temptext stringByAppendingString:self.globalString];
//
...
//            NSLog(@"hasIncrementalChanges：%d",collectionChanges.hasIncrementalChanges);
            ...
            // Log输出
            for(PHAsset *removedasset in collectionChanges.removedObjects){
                NSString *temptext = @"\n监听到相册变化,移除了照片，移除照片的id为：\n";
                //PHFetchResult<PHAsset *> *fetchresult = [PHAsset fetchAssetsWithMediaType:(PHAssetMediaTypeImage) options:(nil)];
                NSString *removedString = removedasset.localIdentifier;
                NSString *readtext = @"\n读取删除相片完成\n\n\n\n";
                self.globalString = [readtext stringByAppendingString:self.globalString];
                self.globalString = [removedString stringByAppendingString:self.globalString];
                self.globalString = [temptext stringByAppendingString:self.globalString];
                self.globalString = [dateString stringByAppendingString:self.globalString];
                //self.LRTextView.text = [removedString stringByAppendingString:self.globalString];
                NSLog(@"已删除：%@",removedasset.localIdentifier);
                
                self.LRTextView.text =self.globalString;
                
                PHImageManager *manager = [PHImageManager defaultManager];
                PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
                [manager requestImageForAsset:removedasset targetSize:CGSizeMake(600, 1000) contentMode:PHImageContentModeDefault options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info){
                   // UIImage *image = result;
                    self.photoImageView.image = result;
                    self.photoImageView = [[UIImageView alloc]initWithFrame:CGRectMake(100, 300, 200 ,400)];
                    NSLog(@"%f",((kScreenWidth - 20.f) / 5.5)  * [UIScreen mainScreen].scale);
...
//            NSLog(@"removedIndexes:%@",collectionChanges.removedIndexes);
            for(PHAsset *insertObject in collectionChanges.insertedObjects){
                NSString *temptext = @"\n监听到相册变化，新增了照片，新增的照片id为：\n";
                //PHFetchResult<PHAsset *> *fetchresult = [PHAsset fetchAssetsWithMediaType:(PHAssetMediaTypeImage) options:(nil)];
                NSString *insertString = insertObject.localIdentifier;
                NSString *readtext = @"\n读取新增相片完成\n\n\n\n";
                self.globalString = [readtext stringByAppendingString:self.globalString];
                self.globalString = [insertString stringByAppendingString:self.globalString];
                self.globalString = [temptext stringByAppendingString:self.globalString];
                self.globalString = [dateString stringByAppendingString:self.globalString];
                //self.LRTextView.text = [insertString stringByAppendingString:self.globalString];
                
                self.LRTextView.text = self.globalString;
                
                PHImageManager *manager = [PHImageManager defaultManager];
                PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
                [manager requestImageForAsset:insertObject targetSize:CGSizeMake(600, 1000) contentMode:PHImageContentModeDefault options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info){
                   // UIImage *image = result;
                    self.photoImageView.image = result;
                    self.photoImageView = [[UIImageView alloc]initWithFrame:CGRectMake(100, 300, 200 ,400)];
                    NSLog(@"%f",((kScreenWidth - 20.f) / 5.5)  * [UIScreen mainScreen].scale);
